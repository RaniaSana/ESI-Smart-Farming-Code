...
public class ZoneCulture extends Zone {
    ...
    public void afficherCulturesParFamille(TypeCulture type) {
        ...
        for (Culture c : filtrees) {
            System.out.println("    [" + c.getCode() + "] " + c.getNom() + " | " + c.getType() + " | " + c.getStadeCroissance());
        }
    }
    ...
    public void afficherDetails() {
        System.out.print("[" + getCode() + "]  " + getNom() + "\t" + "| CropZone \t" + "| " + getStatut() + " | entities: " + cultures.size() + " | crops : ["); 
        
        for (Culture c : cultures) {
            System.out.print( c.getNom() + "(" + c.getStadeCroissance() + "), ");
        }
